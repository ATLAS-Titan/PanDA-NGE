...
if __name__ == '__main__':
	...
	settings = json.load(open("settings.json"))
        ...
        pd_init = {
                'resource'      : resource,
                'runtime'       : 40,  # pilot runtime (min) Set as the maximum allowed in TITAN's fifth bin
                'exit_on_error' : True,
        ...
        for i in range(0, numCUs):
            ...
            cud = CUDef.createGromacsCU(1,settings) ## The number of cores per CU has been set to 1. (hard coded, can be changed). The creation of the CU could stay outside the loop since all the CUs are the same
